...
def pull_blog(url):
    ...
    blogs = driver.find_elements_by_class_name('post')
    for blog in blogs:
        postdates = blog.find_elements_by_class_name('postdate')
        for postdate in postdates:
            article_dates.append(postdate.text)

        article_titles.append(blog.find_element_by_tag_name('h2').text)

        contents = blog.find_elements_by_tag_name('p')
...
pull_blog('http://blogs.rsc.org/tx/')

i = 2
while i < 14:
  pull_blog('http://blogs.rsc.org/tx/page/' + str(i) + '/')
  time.sleep(2)
  i += 1
